...
道を見つけたら以降を省略する
kinectの値をもっと正確に把握する
if文の順番に注意(条件の厳しいもの>優先的に動いてほしいもの)
*/
...
#define MIN_DISTANCE 850	//壁とkinectの取りうる最大距離(曲がり始める最小距離)
#define DANGEROUS_DISTANCE 350	//壁に近づきすぎている距離

int state = -1;		//機体の状態を管理
int roboMove = 0;	//機体の移動方向を管理
double deepth[6];	//ave[]を左右で分割したもの
double ave[3];		//kinectからの深度情報を保存
int moveForward[2];	//前後に何cm進むかを管理
int moveTurn[2];	//旋回角度を管理
