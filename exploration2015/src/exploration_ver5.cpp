...
#define FOURTH_WALL_CHECK 7
#define FIFTH_TURN 8
#define DECIDE_DIRECTION 9
#define GO 10
#define RESET 11

#define LEFT_AND_ONE_STEP 0
#define LEFT_AND_TWO_STEPS 1
#define FORWARD_ONE_STEP 2
#define FORWARD_TWO_STEPS 3
...
void depth_callback(const sensor_msgs::ImageConstPtr& msg) {
	...
	if (state == FOURTH_WALL_CHECK) {
		float ave1, ave2, ave3, ave4;
		aveArea(cv_ptr->image, ave1, ave2, ave3, ave4);
		deepth[6] = ave3;
		deepth[7] = ave4;
		ave[3] = (ave3 + ave4) / 2;
		ROS_INFO("AREA[7]=%lf",deepth[6]);
		ROS_INFO("AREA[8]=%lf",deepth[7]);
		state++;
	}
}
	...
	while(ros::ok()) {
		switch(state){
			...
			case FIFTH_TURN:
				...
				if(ave[0] < DANGEROUS_DISTANCE){
					ROS_INFO("30 degree turn");
					moveTurn[0] = -30;
				}
				else if(deepth[7] < DANGEROUS_DISTANCE){
					ROS_INFO("60 degree turn");
					moveTurn[0] = -60;
				}
				else if(1.2*deepth[1] < deepth[0]){
					ROS_INFO("30 degree turn");
					moveTurn[0] = -30;
				}
			...
			case DECIDE_DIRECTION:
				...
				if(ave[1] < 1.2*1.414*ave[0] && ave[0] < MIN_DISTANCE && MIN_DISTANCE < ave[2]){
					...
					roboMove = LEFT_AND_TWO_STEPS;
					ROS_INFO("To turn to left and take two steps");
				}
				else if(deepth[0] < MIN_DISTANCE && MIN_DISTANCE < deepth[1] && ave[0] < MIN_DISTANCE && ave[1] < MIN_DISTANCE*1.414 && ave[2] < MIN_DISTANCE){
						roboMove = FORWARD_ONE_STEP;
						ROS_INFO("To take one step");
					}
				else if(MIN_DISTANCE < deepth[4] && deepth[5] < MIN_DISTANCE && ave[0] < MIN_DISTANCE && ave[1] < MIN_DISTANCE*1.414 && ave[2] < MIN_DISTANCE){
						roboMove = LEFT_AND_ONE_STEP;
			...
			case GO:
				...
				switch(roboMove){
					case LEFT_AND_ONE_STEP:
						moveForward[0] = 0;  moveTurn[0] = -91;
						moveForward[1] = ONE_STEP;  moveTurn[1] = 0;
						break;

					case LEFT_AND_TWO_STEPS:
						moveForward[0] = 0;  moveTurn[0] = -91;
